...
export async function getPriceIdForActiveUser(email: string) {
  ...
  const query = await sql`SELECT price_id FROM users 
        where email = ${email} 
        AND status = 'active'`
...
export async function hasActivePlan(email: string) {
  ...
  const query = await sql`SELECT price_id, status FROM users 
        where email = ${email} 
...
export async function hasReachedUpUploadLimit(userId: string) {
